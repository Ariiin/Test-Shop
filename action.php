                        ...
                        <div class='col-md-4 col-xs-6' >
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='product-body'>
										...
										<h4 class='product-price header-cart-item-info'>$pro_price 원<del class='product-old-price'> 300,000 원</del></h4>
                        ...
                        <div class='col-md-4 col-xs-6'>
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='product-body'>
										<p class='product-category'>$cat_name</p>
										<h3 class='product-name header-cart-item-name'><a href='product.php?p=$pro_id'>$pro_title</a></h3>
										<h4 class='product-price header-cart-item-info'>$pro_price 원<del class='product-old-price'> 400,000 원</del></h4>
										<div class='product-rating'>
											<i class='fa fa-star'></i>
											<i class='fa fa-star'></i>
											<i class='fa fa-star'></i>
									...
									<div class='add-to-cart'>
										<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'></i> 장바구니 담기</button>
									</div>
								</div>
							</div>
			";
...
if (isset($_POST["count_item"])) {
	//사용자가 로그인하면 사용자 세션 ID를 사용하여 장바구니에있는 항목 수를 계산합니다.
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
	}else{
		// 사용자가 로그인하지 않은 경우 사용자 고유의 IP 주소를 사용하여 장바구니에있는 항목 수를 계산합니다.
...
//데이터베이스에서 장바구니로 장바구니 항목 가져 오기 메뉴
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//사용자가 로그인하면이 쿼리가 실행됩니다.
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//사용자가 로그인하지 않으면이 쿼리가 실행됩니다.
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	if (isset($_POST["getCartItem"])) {
		//드롭 다운 메뉴에 장바구니 항목 표시
		if (mysqli_num_rows($query) > 0) {
			$n=0;
			$total_price=0;
				...
				echo '
                    ...
                    <div class="product-widget">
												...
												<div class="product-body">
													...
													<h4 class="product-price"><span class="qty">'.$n.'</span>'.$product_price.' 원</h4>
												</div>
    ...
    if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//사용자가 로그인하지 않은 경우 "주문하기" 버튼으로 사용자 장바구니 항목 표시
			echo '<div class="main ">
			...
			<form method="post" action="login_form.php">
	               ...
	               <table id="cart" class="table table-hover table-condensed" id="">
    				<thead>
						<tr>
							...
							<th style="width:7%" class="text-center">합계</th>
				...
				<tfoot>
					...
					<tr>
						<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i> 계속 쇼핑하기</a></td>
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							...
							<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success">결제하기</a></td>
								</tr>
							</tfoot>
					...
					echo '
						...
						<form action="checkout.php" method="post">
							...
							<input type="hidden" name="business" value="speednumber1@hanmail.net">
